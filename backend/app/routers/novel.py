...
def construct_site_url(site, novel_id):
    if site == "fanfiction":
        return f"https://www.fanfiction.net/s/{novel_id}"
...
def add_novel(
    ...
    if novel_services.get_novel_by_url(db, novel.url).first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Novel already exists"
        )
    ...
    if not supported_src(novel.url):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Not a supported source url"
        )
...
def update_novelchapters_by_id(
    novel_id: int,
    library_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    url = construct_site_url(site_name, novel.site_story_id)
    py_story = asyncio.run(fetch_story_data_by_url(url, site_name))
    chapter_list = parse_chapter_data(py_story)
    chapter_numbers = [
        chap.chapter_number for chap in get_chapters_by_novel_id(db, novel_id)
    ]
    for idx, chapter in enumerate(chapter_list):
        if chapter.get("chapter_number") not in chapter_numbers:
            chapter["novel_id"] = novel.id
            create_chapter(db, chapter)
...
def update_novel(
    ...
    """
    novel_data = {"title": novel.title, "url": novel.url}
